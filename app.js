...
function initMap() {
  ...
  map = L.map("map", { preferCanvas: true, zoomControl: true }).setView([48.8566, 2.3522], 12);
...
function drawTerrasses(data) {
  ...
  data.forEach((item) => {
    ...
    const popup = `<b>${info["Nom de l'enseigne"] || "Terrasse"}</b><br>${info["Typologie"] || ""}<br><b>Adresse :</b> ${info["Numéro et voie"] || ""} — ${info["Arrondissement"] || ""}`;
    const marker = L.circleMarker([item.geo.lat, item.geo.lon], { radius: 6, color: "#27ae60", fillColor: "#2ecc71", fillOpacity: 0.9 });
    marker.bindPopup(popup);
    layers.terrasses.addLayer(marker);
  });
}

function drawTravaux(data) {
  layers.travaux.clearLayers();
  data.forEach((item) => {
    if (!item.geo) return;
    const info = item.info || {};
    const popup = `<b>${info["Synthèse - Nature du chantier"]}</b><br><b>Début :</b> ${info["Date début du chantier"]}<br><b>Fin :</b> ${info["Date fin du chantier"]}`;
    const marker = L.circleMarker([item.geo.lat, item.geo.lon], { radius: 7, color: "#f1c40f", fillColor: "#f7dc6f", fillOpacity: 0.9 });
    marker.bindPopup(popup);
...
function drawPerturbants(data) {
  ...
  data.forEach((item) => {
    if (!item.geo) return;
    const info = item.info || {};
    const popup = `<b>Objet :</b> ${info["Objet"]}<br><b>Voie(s) :</b> ${info["Voie(s)"]}<br><b>Début :</b> ${info["Date de début"]}<br><b>Fin :</b> ${info["Date de fin"]}`;
    const marker = L.circleMarker([item.geo.lat, item.geo.lon], { radius: 8, color: "#922b21", fillColor: "#e74c3c", fillOpacity: 0.9 });
    marker.bindPopup(popup);
...
function drawBOVP(data) {
  ...
  data.forEach((item) => {
    if (!item.geo) return;
    const popup = `<b>${item.numero || ""}</b><br>${item.titre || ""}<br><b>Type :</b> ${item.type || "—"}<br><b>Adresse :</b> ${item.adresse || "—"}`;
    const m = L.circleMarker([item.geo.lat, item.geo.lon], { radius: 7, color: "#3498db", fillColor: "#5dade2", fillOpacity: 0.9 });
    m.bindPopup(popup);
...
function setupLayerButtons() {
  const layerButtons = {
    terrasses: document.getElementById('btnTerrasses'),
    travaux: document.getElementById('btnTravaux'),
    perturbants: document.getElementById('btnPerturbants'),
    bovp: document.getElementById('btnArretes'),
  };
...
function setupSearchListeners() {
  ...
  searchInput.addEventListener("input", () => {
    ...
    debounceTimer = setTimeout(() => {
      const matches = ruesParis.filter((r) => r.nom.toLowerCase().includes(q)).slice(0, 12);
      acList.innerHTML = "";
      if (!matches.length) {
        acList.style.display = "none";
        return;
      ...
      matches.forEach((rue) => {
        ...
        el.addEventListener("mousedown", (ev) => {
          ...
          searchInput.blur();
          map.flyTo([rue.lat, rue.lon], 17, { duration: 0.8 });
          L.marker([rue.lat, rue.lon]).addTo(map).bindPopup(`📍 ${rue.nom}`).openPopup();
        });
...
// Suppression marqueurs utilisateur
...
function showUpdateNotification() {
  ...
  popup.innerHTML = `<div class="update-notification-content"><div class="update-icon">🗺️</div><h3>Carte mise à jour</h3><p>Les données de la carte ont été actualisées</p><button class="btn-close-notification">OK</button></div>`;
